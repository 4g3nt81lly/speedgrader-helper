import { getGradingStates } from '#content/stores/gradingStates.actions';
...
export function restoreSGFeedback() {
	const gradingStates = getGradingStates();
	if (!gradingStates) return;

	for (const { sgState, sgElements } of Object.values(gradingStates)) {
		const { pointsInput, commentsTextarea } = sgElements;
